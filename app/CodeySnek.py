...
def move():
    ...
    if HoldDirection == "left":
        ...
        if not isSafe(SnekHeadX-1, SnekHeadY, jsonData['width'], jsonData['height'], jsonData):
            HoldDirection = "down"
#End of safety Checks. Could be optimized? 
...
def isSafe(x,y,width, height,json):
    ...
    if y >= height or y<0:
        return False    
    for snake in json['snakes']['data']: #iterates through all the snakes to see if next move is safe 
...
#Checks for food yo! 
    CloseFood = 0
    jsonFood = jasonData['food']['data']
    for k,Food in enumerate(jasonData['food']['data']):
        print ('X =', Food['x'], 'Y=', Food['y'])
        if (abs(SnekX - Food['x'])+abs(SnekY - Food['y'])) < (abs(SnekX - jsonFood[CloseFood]['x']) + abs(SnekY - jsonFood[CloseFood]['y'])):
            CloseFood = k
    if SnekX > jasonData['food']['data'][CloseFood]['x']:
        return "left" 
    if SnekX < jasonData['food']['data'][CloseFood]['x']:
        return "right"
    if SnekY > jasonData['food']['data'][CloseFood]['y']:
        return "up"
    if SnekY < jasonData['food']['data'][CloseFood]['y']:
        return "down"
